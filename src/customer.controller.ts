...
import { ApiBody, ApiOperation, ApiBearerAuth, ApiResponse } from '@nestjs/swagger';
...
export class CustomerController {
    ...
    async heartbeat(@Body() heartBeat: HeartBeatRequestDto): Promise<HeartBeatResponseDto> {
        ...
        if(room.isPowerOn == false && heartBeat.power == true) {
            this.scheduleService.insertOrUpdateScheduleById(room.id, heartBeat.current, heartBeat.target, heartBeat.wind);
            var response = new HeartBeatResponseDto();
            response.cost = previousFee.bill;
            response.status = 0;
